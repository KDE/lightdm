...
ConfigWrapper::ConfigWrapper(const KUrl &configPath, const KSharedConfigPtr &config, QObject *parent) :
    QObject(parent)
{

    qDebug() << "config wrapper loading" << configPath.toLocalFile();

    QFile xmlFile(configPath.toLocalFile());
